...
void nvmed_io_polling(NVMED_HANDLE* nvmed_handle, u16 target_id) {
	...
	while(1) {
		...
		for (;;) {
			if((cqe->status & 1) == nvmed_queue->cq_phase)
				break;
			if(iod->status == IO_COMPLETE) break;
		}
...
ssize_t nvmed_pread(NVMED_HANDLE* nvmed_handle, void* buf, size_t count, off_t offs) {
	ssize_t ret;
...
ssize_t nvmed_pwrite(NVMED_HANDLE* nvmed_handle, void* buf, size_t count, off_t offs) {
	ssize_t ret;
