...
static void _fdb_init_file_config(const fdb_config *config,
	...
	// HJKIM
	fconfig->need_init = config->need_init;
	storage_engine = config->storage_engine;
	if(storage_engine == NVMeDirect) {
		storage_data.nvmedirect.nvmed = config->storage_data.nvmedirect.nvmed;
	}
}
...
fdb_commit_start:
    ...
    handle->last_hdr_bid = filemgr_alloc(handle->file, &handle->log_callback);
		filemgr_modify_filesize(handle->file, handle->last_hdr_bid * handle->file->blocksize,
...
LIBFDB_API
uint64_t fdb_stat_get_sum(fdb_kvs_handle *handle,
											int attr) {
	return _kvs_stat_get_sum(handle->file, (kvs_stat_attr_t)attr);
}
