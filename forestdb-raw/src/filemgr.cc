...
ssize_t filemgr_write_blocks(struct filemgr *file, void *buf, unsigned num_blocks, bid_t start_bid) {
    ...
    if (file->encryption.ops == NULL) {
        rv = file->ops->pwrite(file->fd, buf, nbytes, offset);
		// HJKIM
		if(rv > 0) filemgr_modify_filesize(file, nbytes, offset);
		return rv;
    } else {
        ...
        if (status != FDB_RESULT_SUCCESS)
            return status;
        rv = file->ops->pwrite(file->fd, encrypted_buf, nbytes, offset);
		// HJKIM
		if(rv > 0) filemgr_modify_filesize(file, nbytes, offset);
		return rv;
    }
                                   ...
                                   err_log_callback *log_callback)
{
    ...
    if (sb_ops.init && sb_ops.get_default_config && sb_ops.read_latest) {
        ...
        if (filemgr_get_pos(file) || !file->need_init) { 
            // existing file
                                 ...
                                 err_log_callback *log_callback)
{
    ...
    *file->config = *config;
	...
	// HJKIM - offset
    cs_off_t offset = 0;
	if(file->sb) offset = file->sb->num_used_blocks * file->blocksize;
	if(config->need_init)
		offset = 0;
    ...
    if (file->config && file->config->options & FILEMGR_CREATE_CRC32) {
		...
		file->need_init = config->need_init;
    ...
    do { // repeat until both superblock and DB header are correctly read
		...
		// HJKIM : Modify file offset (eof or file size)
				
		if(!config->need_init) {
			offset = (file->sb->num_used_blocks) * file->blocksize;
			atomic_init_uint64_t(&file->last_commit, offset);
			atomic_init_uint64_t(&file->pos, offset);
		}
...
bid_t filemgr_alloc(struct filemgr *file, err_log_callback *log_callback)
{
    ...
    if (global_config.ncacheblock <= 0) {
		...
		// HJKIM
		if(rv > 0) filemgr_modify_filesize(file, 1, (bid+1) * file->blocksize - 1);
                            ...
                            bid_t *end, err_log_callback *log_callback)
{
    ...
    *end = *begin + nblock - 1;
    ...
    if (global_config.ncacheblock <= 0) {
		...
		// HJKIM
		if(rv > 0) filemgr_modify_filesize(file, 1, atomic_get_uint64_t(&file->pos) - 1);
        _log_errno_str(file->ops, log_callback, (fdb_status) rv, "WRITE", file->filename);
    }
    spin_unlock(&file->lock);
}
    ...
    if (bid == nextbid) {
        ...
        *end = *begin + nblock - 1;
        ...
        if (global_config.ncacheblock <= 0) {
			...
			// HJKIM
			if(rv > 0) filemgr_modify_filesize(file, 1, atomic_get_uint64_t(&file->pos));
			_log_errno_str(file->ops, log_callback, (fdb_status) rv, "WRITE", file->filename);
        }
    }else{
        *begin = BLK_NOT_FOUND;
    ...
    if (global_config.ncacheblock > 0) {
        ...
        if (len == file->blocksize) {
            ...
            if (r == 0) {
				...
				// HJKIM - offset
				int64_t cur_file_pos = file->sb->num_used_blocks * file->blocksize;
                if (cur_file_pos < 0) {
                    _log_errno_str(file->ops, log_callback,
		...
		// HJKIM
		if(r > 0) filemgr_modify_filesize(file, len, pos);
    ...
    if (e) {
        ...
        *file->config = *config;
        ...
        if (file->fd < 0) {
			...
			// HJKIM - offset
			cs_off_t offset = file->sb->num_used_blocks * file->blocksize;
			if (offset < 0) {
                if (!destroy_file_set) { // top level or non-recursive call
                    hash_free(destroy_set);
                }
...
// HJKIM
void filemgr_modify_filesize(filemgr* file, size_t count, cs_off_t offset) {
	
	ssize_t num_blocks;

	num_blocks = (offset+count) / file->blocksize;
	if((offset+count)%file->blocksize > 0)
		num_blocks++;

	if(num_blocks > file->sb->num_used_blocks)
		file->sb->num_used_blocks = num_blocks;
