                    ...
                    err_log_callback * log_callback)
{
	...
	// # used blocks - HJKIM
	enc_u64 = _endian_encode(file->sb->num_used_blocks);
	memcpy(buf + offset, &enc_u64, sizeof(enc_u64));
    offset += sizeof(enc_u64);
    ...
    if (r != real_blocksize) {
		...
		filemgr_modify_filesize(file, real_blocksize, sb_no * real_blocksize);
                                    ...
                                    err_log_callback *log_callback)
{
	...
	memcpy(&enc_u64, buf + offset, sizeof(enc_u64));
	offset += sizeof(enc_u64);
    sb->num_used_blocks = _endian_decode(enc_u64);
...
INLINE void _sb_copy(struct superblock *dst, struct superblock *src)
{
    ...
    dst->num_used_blocks = src->num_used_blocks;
                   ...
                   err_log_callback * log_callback)
{
    ...
    if (file->need_init == false && filemgr_get_pos(file) > 0) {
